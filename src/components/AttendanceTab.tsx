...
export const AttendanceTab: React.FC<AttendanceTabProps> = ({ onOpenLeaveModal }) => {
  ...
  const monthlyStats = useMemo(() => {
    ...
    const attended = monthLogs.filter((l) => isValidClockTime(l.clockIn) && l.punctualityStatus !== 'ABSENT');
        ...
        <div className="grid grid-cols-7 gap-1.5 text-center">
          ...
          {Array.from({ length: totalDaysInMonth }).map((_, i) => {
            ...
            if (log) {
              ...
              // punctualityStatus is authoritative; only fall back to the
              // (possibly stale) statusColor when punctualityStatus is unset.
              if (st === 'absent' || (!st && log.statusColor === 'red')) {
        ...
        {filteredLogs.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredLogs.map((log, idx) => {
              ...
              // (possibly stale) statusColor when punctualityStatus is unset.
              const isAbsent =
                log.punctualityStatus === 'ABSENT' ||
                (!log.punctualityStatus && log.statusColor === 'red');
