...
import { isValidClockTime } from '../lib/time';
...
export const HomeTab: React.FC<HomeTabProps> = ({ onOpenLeaveModal }) => {
  ...
  useEffect(() => {
    const hasClockIn = isValidClockTime(activeLog?.clockIn);
    const hasClockOut = isValidClockTime(activeLog?.clockOut);

    if (!activeLog || !hasClockIn || hasClockOut) {
      if (hasClockOut && activeLog?.durationText) {
  ...
  const handleToggleClock = async () => {
    try {
      ...
      // CASE 1: CLOCK OUT (Active log exists without a valid clockOut time)
      if (activeLogKey && activeLog && !isValidClockTime(activeLog.clockOut)) {
      ...
      if (activeLog && isValidClockTime(activeLog.clockOut)) {
  ...
  const isClockedIn = Boolean(
    activeLog && !isAbsent && isValidClockTime(activeLog.clockIn) && !isValidClockTime(activeLog.clockOut)
  );

  const isShiftCompleted = Boolean(
    activeLog &&
      !isAbsent &&
      isValidClockTime(activeLog.clockIn) &&
      isValidClockTime(activeLog.clockOut)
  );
